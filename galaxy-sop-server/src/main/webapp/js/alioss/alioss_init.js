		...
		$("#" + ossClient.uploadBtn).click(function() {  //上传按钮点击事件开始上传
			...
			if (ossClient.uploadObject.uploadMode == "oss") {
				...
				if (!ossClient.getLocalFormParam) {
					...
					formParams = ossClient.getLocalFormParam(up, up.files[0],ossClient.fileKey);
				}
			}
  ...
  var ossClient = {
		  ...
		  uploadInit : function(data){
			  ...
			  if(data){
				  ...
				  if(!data.fileKey){
					  data.fileKey = undefined;
>>>>>>> branch 'develop2' of http://git.gi.com/galaxy-star/galaxy-sop.git
					}
<<<<<<< HEAD
				}
			} else {
...
<<<<<<< HEAD
				...
				sendPostRequestByJsonObj(ossClient.ossUrl, form,
						function(data) {
