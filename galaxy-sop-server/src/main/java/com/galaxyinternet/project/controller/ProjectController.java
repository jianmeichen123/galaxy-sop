...
public class ProjectController extends BaseControllerImpl<Project, ProjectBo> 
{
	...
	public ResponseData<Project> searchProject(HttpServletRequest request, @RequestBody ProjectBo project)
	{
		...
		project.setCreateUid((project.getCreateUid() != null && project.getCreateUid().longValue() == 0L) ? null
				: project.getCreateUid());
		project.setProjectDepartid(
				(project.getProjectDepartid() != null && project.getProjectDepartid().longValue() == 0L) ? null
						: project.getProjectDepartid());
		...
		Page<Project> pageProject = projectService.queryPageList(project, new PageRequest(project.getPageNum(),
	...
	@ResponseBody
	@ApiOperation("添加项目")
	@RequestMapping(value = "/insertProject", produces = MediaType.APPLICATION_JSON_VALUE)
			...
			throws ParseException
	{
		...
		if (num > 0)
		{
			...
			ControllerUtils.setRequestParamsForMessageTip(request, projectName, project.getId(), "2");
	...
	public ResponseData<Project> selectProject(@PathVariable("pid") String pid, HttpServletRequest request)
	{
		...
		if (project != null)
		{
			...
			{
				String name = DictEnum.industryOwn.getNameByCode(project.getIndustryOwn().toString());
				if (name != null)
				{
			...
			if (null != project.getFinanceMode() && !"".equals(project.getFinanceMode()))
			{
				...
				if (project.getFinanceMode().equals("financeMode:1")
						|| project.getFinanceMode().equals("financeMode:2"))
				{
					jointDelivery.setDeliveryType(project.getFinanceMode());
	...
	public ResponseData<Project> queryAllProjects(HttpServletRequest request, @RequestBody ProjectBo project)
	{
		...
		try
		{
			...
			Page<Project> pageProject = projectService.queryPageList(project,
					new PageRequest(project.getPageNum(), project.getPageSize(), direction, property));
	 ...
	 *//*
		 ...
		 * @ResponseBody
		 * 
		 * @RequestMapping(value = "/app", method = RequestMethod.POST, produces
		 * = MediaType.APPLICATION_JSON_VALUE) public ResponseData<PersonPoolBo>
		 * addProjectPerson(
		 * 
		 * @RequestBody PersonPoolBo pool, HttpServletRequest request) {
		 * ResponseData<PersonPoolBo> responseBody = new
		 * ResponseData<PersonPoolBo>(); if (pool.getProjectId() == null ||
		 * pool.getProjectId() <= 0 || pool.getPersonName() == null ||
		 * pool.getPersonTelephone() == null) { responseBody.setResult(new
		 * Result(Status.ERROR, null, "必要的参数丢失!")); return responseBody; } User
		 * user = (User) getUserFromSession(request); Project p =
		 * projectService.queryById(pool.getProjectId()); //
		 * 项目创建者用户ID与当前登录人ID是否一样 if (p != null && user.getId().doubleValue() !=
		 * p.getCreateUid().doubleValue()) { responseBody.setResult(new
		 * Result(Status.ERROR, null, "没有权限为该项目添加团队成员!")); return responseBody;
		 * } try { pool.setCreatedTime(System.currentTimeMillis()); Long id =
		 * personPoolService.addProjectPerson(pool); if (id > 0) { responseBody
		 ...
		 * p.getProjectName(), p.getId()); } } catch (Exception e) {
		 * _common_logger_.error("添加团队异常 ",e.getMessage()); } return
		 * responseBody; }
		 */
	...
	public ResponseData<PersonPoolBo> deleteProjectPerson(@PathVariable("id") Long id,
			@PathVariable("projectId") Long projectId, HttpServletRequest request)
	{
		ResponseData<PersonPoolBo> responseBody = new ResponseData<PersonPoolBo>();
		if (projectId == null)
		{
	...
	public ResponseData<PersonPool> queryProjectPerson(HttpServletRequest request,
			@RequestBody PersonPoolBo personPoolBo)
	{
		ResponseData<PersonPool> responseBody = new ResponseData<PersonPool>();
		try
		{
			Page<PersonPool> pageList = personPoolService.queryPageListByPid(personPoolBo,
					new PageRequest(personPoolBo.getPageNum(), personPoolBo.getPageSize()));
	...
	public ResponseData<PersonPool> queryPersonListToTask(HttpServletRequest request,
			@RequestBody PersonPoolBo personPoolBo)
	{
		ResponseData<PersonPool> responseBody = new ResponseData<PersonPool>();
	...
	public ResponseData<Project> store(HttpServletRequest request, @PathVariable("pid") Long pid)
	{
		...
		{
			_common_logger_.info("old stockTransfer:" + project.getStockTransfer() + ", new stockTransfer:" + r);
		}
	 ...
	 * @author yangshuhua
	 * @deprecated Use
	 *             {@link com.galaxyinternet.project_process.controller.ProjectFlowController#stageChange}instead.
	 */
	@com.galaxyinternet.common.annotation.Logger
	@ResponseBody
	...
	public ResponseData<Project> inCeoMeetingPool(HttpServletRequest request, @PathVariable("pid") Long pid)
	{
		...
		try
		{
			...
			if (!tm.getStatus().equals(DictEnum.meetingResult.待定.getCode()))
			{
				...
				ControllerUtils.setRequestParamsForMessageTip(request, null, project.getProjectName(), project.getId(),
						UrlNumber.one);
			} else
			{
	...
	public Result validate(String progress, Project project, User user)
	{
		...
		} else if (project.getProjectStatus().equals(DictEnum.meetingResult.否决.getCode())
				|| project.getProjectStatus().equals(DictEnum.projectStatus.YFJ.getCode()))
		{ // 字典 项目状态 = 会议结论 关闭
			return new Result(Status.ERROR, null, "项目已关闭!");
		} else if (project.getProjectStatus().equals(DictEnum.projectStatus.YTC.getCode()))
		{ // 字典 项目状态 = 会议结论 关闭
	...
	public ResponseData<Project> breakproject(@RequestBody OperationLogs param, HttpServletRequest request)
	{
		ResponseData<Project> responseBody = new ResponseData<Project>();
		...
		try
		{
			...
			responseBody.setResult(new Result(Status.OK, ""));
			ControllerUtils.setRequestParamsForMessageTip(request, project.getProjectName(), project.getId(), null,
					false, null, param.getReason(), null);
		} catch (Exception e)
		{
	...
	@ResponseBody
	public ResponseData<MeetingRecord> checkCanUse(HttpServletRequest request, Integer index, Long projectId,
			String projectType)
	{
		...
		try
		{
			...
			{ // 立项会阶段，在排期池中时， 申请立项会排期按钮 不可用
				 ...
				 * //尽职调查去掉文件验证 if (projectType == null) {
				 * responseBody.setResult(new Result(Status.ERROR, null,
				 * "入参失败")); return responseBody; } // 验证文档是否齐全 SopFile file =
				 * new SopFile(); file.setProjectId(projectId);
				 * file.setFileValid(1);
				 * file.setProjectProgress(DictEnum.projectProgress.尽职调查.getCode
				 * ()); List<SopFile> files = sopFileService.queryList(file);
				 * 
				 * boolean allHas = true; if (files != null &&
				 * ((projectType.equals(DictEnum.projectType.投资 .getCode()) &&
				 * files.size() == 4) || (projectType
				 * .equals(DictEnum.projectType.创建.getCode()) && files .size()
				 * == 2))) { for (SopFile f : files) { if (f.getFileKey() ==
				 * null || "".equals(f.getFileKey().trim())) { allHas = false;
				 * break; } } } else { allHas = false; }
				 * responseBody.setResult(new Result(Status.OK, null, allHas));
				 * 
				 * }
				 */else if (index == 7)
			{ // 投决会阶段，在排期池中时， 申请投决会排期按钮 不可用
				MeetingScheduling meetSchedu = new MeetingScheduling();
				meetSchedu.setProjectId(projectId);
		...
		try
		{
			...
			if (list != null && !list.isEmpty())
			{
				...
				} else
				{
					...
					_common_logger_.error("checkHasPool 数据返回错误,应返回一条数据  " + GSONUtil.toJson(list));
				}
			} else
			{
				responseBody.setResult(new Result(Status.OK, ""));
	...
	public ResponseData<MeetingRecord> checkPassMeet(@RequestBody MeetingRecord query)
	{
		...
		try
		{
			...
			if (list != null && !list.isEmpty())
			{
				...
				{
					responseBody.setResult(new Result(Status.ERROR, null, "数据返回错误"));
					_common_logger_.error("checkPassMeet 数据返回错误,应返回一条数据  " + GSONUtil.toJson(list));
				}
			} else
			{
				responseBody.setResult(new Result(Status.OK, ""));
	...
	public ResponseData<ProjectQuery> updateCommonFile(ProjectQuery p, HttpServletRequest request)
	{
		...
		if (p.getPid() == null || p.getStage() == null
				|| !SopConstant._progress_pattern_.matcher(p.getStage()).matches() || p.getParseDate() == null)
		{
			responseBody.setResult(new Result(Status.ERROR, null, "必要的参数丢失!"));
			return responseBody;
		}
	...
	public ResponseData<Dict> getFinanceStatusByParent(@PathVariable String id, HttpServletRequest request)
	{
		ResponseData<Dict> responseBody = new ResponseData<Dict>();
		List<Dict> dicts = new ArrayList<Dict>();
		Dict dict = null;
	...
	public ResponseData<MeetingScheduling> searchSchedulingList(HttpServletRequest request,
			@PathVariable("type") Integer type, @RequestBody MeetingSchedulingBo query)
	{
		...
		PageRequest pageable = new PageRequest(0, 10, direction, property);
		...
		try
		{
			...
			if (pids != null && pids.size() > 0)
			{
				...
				pageList = meetingSchedulingService.getMeetingList(query,
						new PageRequest(query.getPageNum(), query.getPageSize(), direction, property));
	...
	public ResponseData<MeetingScheduling> updateReserveTime(HttpServletRequest request,
			@RequestBody List<MeetingScheduling> query)
	{
		...
		} else
		{
			for (MeetingScheduling meeting : mslist)
			{
				...
				if (DictEnum.meetingType.投决会.getCode().equals(meeting.getMeetingType()))
				{
					flag = true;
...
/*						sendTaskProjectEmail(request, pj, messageInfo, userlist, null, null, 0, UrlNumber.three);*/
	 ...
	 *            0：取消发送邮件 1:更新或新增邮件
	 */
	public void sendTaskProjectEmail(HttpServletRequest request, Project pj, String messageInfo, List<User> userlist,
			Timestamp reserveTimeStart, Timestamp reserveTimeEnd, Integer type, UrlNumber number)
	{
		...
		if (!userlist.isEmpty())
		{
			...
			for (User user : userlist)
			{
				 ...
				 * pj.getProjectName(), pj.getId(), number);
				 */
			...
			sendMailToTZJL(request, type, address, "", pj.getProjectCode(), pj.getProjectName(), messageInfo,
					reserveTimeStart, reserveTimeEnd);
		}
	}
	...
	public ResponseData<MeetingScheduling> updateReserveTimeByApp(HttpServletRequest request,
			@RequestBody MeetingScheduling query)
	{
		ResponseData<MeetingScheduling> responseBody = new ResponseData<MeetingScheduling>();
		if (query == null || query.getMeetingType() == null || "".equals(query.getMeetingType().trim())
				|| query.getId() == null)
		{
			responseBody.setResult(new Result(Status.ERROR, null, "无操作数据!"));
			return responseBody;
		...
		try
		{
			MeetingScheduling oldMs = meetingSchedulingService.queryById(query.getId());
			String mestr = "";
			if (DictEnum.meetingType.投决会.getCode().equals(query.getMeetingType()))
			{
				mestr = DictEnum.meetingType.投决会.getName();
			}
			if (DictEnum.meetingType.立项会.getCode().equals(query.getMeetingType()))
			{
				mestr = DictEnum.meetingType.立项会.getName();
			}
			if (DictEnum.meetingType.CEO评审.getCode().equals(query.getMeetingType()))
			{
				mestr = DictEnum.meetingType.CEO评审.getName();
			}
			String messageInfo = mestr + "排期时间为";
			if (oldMs.getReserveTimeStart() != null && query.getReserveTimeStart() != null)
			{
			...
			{
				// 取消排期时间
				if (query.getReserveTimeStart() == null && query.getReserveTimeEnd() == null)
				{
					query.setScheduleStatus(0);
				...
				{
					// 更新会议时间
					if (oldMs.getReserveTimeStart().getTime() != query.getReserveTimeStart().getTime()
							|| oldMs.getReserveTimeEnd().getTime() != query.getReserveTimeEnd().getTime())
					{
						meetingSchedulingService.updateByIdSelective(query);
						sendTaskProjectEmail(request, pj, messageInfo, userlist, query.getReserveTimeStart(),
								query.getReserveTimeEnd(), 1, UrlNumber.two);
					}
				}
			} else
			{
				...
				if (query.getReserveTimeStart() != null && query.getReserveTimeEnd() != null)
				{
					meetingSchedulingService.updateByIdSelective(query);
					sendTaskProjectEmail(request, pj, messageInfo, userlist, query.getReserveTimeStart(),
							query.getReserveTimeEnd(), 1, UrlNumber.one);
				}
	 ...
	 * 发送邮件
	 */
	public String sendMailToTZJL(HttpServletRequest request, Integer type, String toAddress, String tzjlName,
			String projectinfoCode, String projectinfoName, String messageInfo, Date meetingTimestart,
			Date meetingTimeend)
	{
		...
		/*
		 * String[] to = toAddress.split(";"); if (to != null && to.length == 1)
		 * { int atIndex = toAddress.lastIndexOf("@"); tzjlName =
		 * toAddress.substring(0, atIndex) + ":<br>您好!"; }
		 */
		tzjlName = "您好!";
		if (type == 0)
		{
			content = MailTemplateUtils.getContentByTemplate(Constants.MAIL_PQC_CONTENT_CANCLE);
			content = PlaceholderConfigurer.formatText(content, tzjlName, projectinfoCode, projectinfoName,
					messageInfo);
		} else
		{
			content = PlaceholderConfigurer.formatText(content, tzjlName, projectinfoCode, projectinfoName, messageInfo,
					starttime, endtime);
		}
		boolean success = SimpleMailSender.sendMultiMail(toAddress, "会议排期通知", content.toString());
	 ...
	 * 
	 * @return map.add = y/n/k y:可添加会议 n:不可以添加，需要排期 k:不允许添加 v:不可以添加，需要秘书排期操作
	 *         map.meettype = meetingType:2 map.butname = '添加立项会会议记录' /
	 *         '申请立项会排期'
	 */
	@ResponseBody
	...
	public ResponseData<BaseEntity> initProMeetBut(@PathVariable Long id, HttpServletRequest request)
	{
		...
		try
		{
			...
			/*
			 * GJZ("跟进中" , "projectStatus:0"), THYY("投后运营" , "projectStatus:1"),
			 * YFJ("已否决" ,"projectStatus:2"), YTC("已退出" ,"projectStatus:3");
			 */
			if (user.getId().intValue() != proinfo.getCreateUid().intValue()
					|| proinfo.getProjectStatus().equals("meetingResult:3")
					|| proinfo.getProjectStatus().equals(DictEnum.projectStatus.YFJ.getCode())
					|| proinfo.getProjectStatus().equals(DictEnum.projectStatus.YTC.getCode()))
			{
				resultMap.put("add", "k");
			...
			int indez = Integer
					.parseInt(proinfo.getProjectProgress().substring(proinfo.getProjectProgress().length() - 1));
			...
			/*
			 * 会议通过 待定 内评会： 直接到下一个阶段 一直循环添加会议记录 ceo评审： 申请立项会排期 申请ceo排期 立项会：
			 * 直接到下一个阶段 申请立项会排期 投决会： 直接到下一个阶段 申请投决会排期
			 */
			if (add == null && meettype != null)
			{
				...
				{ // 待定，会议排期池校验
					...
					} else
					{
						...
						/*
						 * `sop_meeting_scheduling` 0 2 ms -1 排期but-->0 ms-->1
						 * `sop_meeting_record` 待定 待定
						 */
						if (scheduling.getScheduleStatus() == 0)
						{ // 0 等秘书排期操作
							add = "v";
							butname = "待排期";
						} else if (scheduling.getScheduleStatus() == 1)
						{ // 1表示已排期,可以添加会议记录
							add = "y";
							// butname = "添加"+butname+"会议记录";
							butname = "添加会议记录";
						} else if (scheduling.getScheduleStatus() == 2)
						{ // 1表示已排期,可以添加会议记录
							add = "n";
	...
	public String toAppropriation(@PathVariable("pid") Long pid,
			@PathVariable("searchPartMoney") String searchPartMoney, HttpServletRequest request)
	{
		Project proinfo = new Project();
		proinfo = projectService.queryById(pid);
		request.setAttribute("pid", pid);
	...
	public String toTabProjectInfo(HttpServletRequest request, @PathVariable Long projectId)
	{
		if (projectId != null && projectId.intValue() != 0)
		{
			...
			if (null != project.getFinanceMode() && !"".equals(project.getFinanceMode()))
			{
				...
				jointDelivery.setProjectId(project.getId());
				jointDelivery.setDeliveryType(project.getFinanceMode());
				if (null != project.getFinanceMode() && (project.getFinanceMode().equals("financeMode:1")
						|| project.getFinanceMode().equals("financeMode:2")))
				{
					jointDelivery.setDeliveryType(project.getFinanceMode());
					queryList = jointDeliveryService.queryList(jointDelivery);
	...
	public String toRight(@PathVariable("projectId") Long projectId, HttpServletRequest request)
	{
		...
		try
		{
			InformationProgress reportProgress = informationProgressService.initUsersAllReportProgressOfPro(null,
					projectId);
	...
	/**
	 * 添加团队成员 1、personPool 2、personLearn 3、personWork 4、projectPerson
	 */
	@com.galaxyinternet.common.annotation.Logger
	@ResponseBody
	...
	public ResponseData<PersonPool> addProPersonAndPerInfo(@RequestBody PersonPool pool, HttpServletRequest request)
	{
		...
		try
		{
			...
			if (pool.getProjectId() == null || pool.getProjectId() <= 0 || pool.getPersonName() == null
			...
			if (personLearn != null && personLearn.size() > 0)
			{
				...
				{
					if (StringUtils.isNotEmpty(pl.getOverDateStr()) && "至今".equals(pl.getOverDateStr()))
					{
						pl.setOverDate(null);
			...
			if (personWork != null && personWork.size() > 0)
			{
				...
				{
					if (StringUtils.isNotEmpty(pw.getOverWorkStr()) && "至今".equals(pw.getOverWorkStr()))
					{
						pw.setOverWork(null);
	...
	public void exportProjectGrade(HttpServletRequest request, HttpServletResponse response)
	{
		...
		try
		{
			...
			SopReportModal modal = reportService.createReport(setFormat,
					request.getSession().getServletContext().getRealPath(""), tempfilePath, suffix);
	...
	public List<ProjectData> setData(List<Project> list, Project project)
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			pd.setProjectDescribeFinancing(
					null == p.getProjectDescribeFinancing() ? "" : p.getProjectDescribeFinancing());
	...
	public ResponseData<Project> deletePro(@RequestBody Project param, HttpServletRequest request)
	{
		ResponseData<Project> responseBody = new ResponseData<Project>();
		...
		try
		{
			...
			if (project == null || project.getCreateUid() == null)
			{
				responseBody.setResult(new Result(Status.ERROR, null, "删除的项目不存在"));
				return responseBody;
			}
			project.setIsdelete(1);
			if (null != param.getDeleteReason() && !"".equals(param.getDeleteReason()))
			{
				project.setDeleteReason(param.getDeleteReason());
			}
			...
			params.put(PlatformConst.REQUEST_SCOPE_RECORD_IDS, Arrays.asList(project.getId()));
			params.put(PlatformConst.REQUEST_SCOPE_MESSAGE_REASON, project.getDeleteReason());
		...
		{
			responseBody.setResult(new Result(Status.ERROR, null, "delete project faild"));
