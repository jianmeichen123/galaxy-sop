...
public enum OperationLogType {
	...
	//接触访谈阶段
	//添加访谈的弹出框中点击‘保存’
	ADD_INTERVIEW("", OperType.ADD.getOperationType(), "访谈记录",SopStage.TOUCH_INTERVIEW.getStageName()), 
	//点击‘启动内部评审’
	SUBMIT_INNER_REVIEW_SCHEDULE("", OperType.SUBMIT.getOperationType(), "内部评审",SopStage.TOUCH_INTERVIEW.getStageName()), 
	
	//内部评审阶段
	//添加会议记录的弹出框中点击‘保存’。
	INNER_REVIEW_SCHEDULE("", OperType.ADD.getOperationType(), "会议记录",SopStage.INNER_REVIEW_SCHEDULE.getStageName()), 
	
	//CEO评审阶段
	//添加会议记录的弹出框中点击‘保存’。
	CEO_REVIEW_SCHEDULE("", OperType.ADD.getOperationType(), "会议记录",SopStage.CEO_REVIEW_SCHEDULE.getStageName()), 
	//点击申请立项会排期
	ADD_PROJECT_SCHEDULE("", OperType.SUBMIT.getOperationType(), "立项会排期",SopStage.CEO_REVIEW_SCHEDULE.getStageName()), 
	
	//立项会阶段
	//添加会议记录的弹出框中点击‘保存’。
	ADD_MEETING_RECORD("", OperType.ADD.getOperationType(), "会议记录",SopStage.PROJECT_SCHEDULE.getStageName()), 
	//点击申请立项会排期
	...
	//上传投资意向书
	UPLOAD_INVESTMENT_TEMPLATE("", OperType.UPLOAD.getOperationType(), "投资意向书",SopStage.INVESTMENT_INTENT.getStageName()),
	//更新投资意向书
	UPDATE_INVESTMENT_TEMPLATE("", OperType.UPDATE.getOperationType(), "投资意向书",SopStage.INVESTMENT_INTENT.getStageName()),
	//上传签署证明	
	SIGNED_CERTIFICATE("", OperType.UPLOAD.getOperationType(), "签署证明",SopStage.INVESTMENT_INTENT.getStageName()),

	//尽职调查阶段
	//上传业务尽职调查报告	  
	UPLOAD_DUE_DILIGENCE_INVESTIGATION("", OperType.UPLOAD.getOperationType(), "业务尽职调查报告",SopStage.DUE_DILIGENCE_INVESTIGATION.getStageName()),
	//点击申请投决会排期	
	APPLY_VOTE_SCHEDULE("", OperType.SUBMIT.getOperationType(), "投决会排期",SopStage.DUE_DILIGENCE_INVESTIGATION.getStageName()),
	//尽职调查报告催办
	REMIND_VOTE_SCHEDULE("", OperType.REMINDER.getOperationType(), "法务尽职调查报告",SopStage.DUE_DILIGENCE_INVESTIGATION.getStageName()),
	
	//投决会阶段
	//添加会议纪要的弹出框中点击“保存”
	ADD_VOTE_DECISION_MEETING_NOTE("", OperType.ADD.getOperationType(), "会议记录",SopStage.VOTE_DECISION_MEETING.getStageName()),
	...
	//投资协议上传
	INVESTMENT_AGREEMENT_LOOK("", OperType.DOWNLOAD.getOperationType(), "投资协议",SopStage.INVESTMENT_AGREEMENT.getStageName()),
	//投资协议更新
	UPDATE_INVESTMENT_AGREEMENT("", OperType.UPDATE.getOperationType(), "投资协议",SopStage.INVESTMENT_AGREEMENT.getStageName()),
	//股权转让协议上传
	UPLOAD_EQUITY_TRANSFER_AGREEMENT("", OperType.UPLOAD.getOperationType(), "股权转让协议",SopStage.INVESTMENT_AGREEMENT.getStageName()),
	//股权转让协议更新
	UPDATE_EQUITY_TRANSFER_AGREEMENT("", OperType.UPDATE.getOperationType(), "股权转让协议",SopStage.INVESTMENT_AGREEMENT.getStageName()),
	//股权转让协议签署凭证上传
	UPLOAD_EQUITY_TRANSFER_AGREEMENT_SIGNED_CERTIFICATE("", OperType.UPLOAD.getOperationType(), "股权转让协议签署凭证",SopStage.INVESTMENT_AGREEMENT.getStageName()),
	//股权转让协议签署凭证更新
	...
	//投资协议签署凭证上传
	UPLOAD_EQUITY_INVESTMENT_SIGNED_CERTIFICATE("", OperType.UPLOAD.getOperationType(), "股权转让协议签署凭证",SopStage.INVESTMENT_AGREEMENT.getStageName()),
	//投资协议签署凭证更新
